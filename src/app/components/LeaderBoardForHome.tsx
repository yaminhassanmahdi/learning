...
export default function WeeklyLeaderboard() {
    ...
    // --- Render Logic ---
    return (
        <div className="flex flex-col md:h-auto my-4 justify-center items-center  relative w-fit mx-auto bg-zinc-800  scrollbar-hide">
            <div className="w-full max-w-lg h-[100%] p-4 sm:p-4 bg-zinc-900 rounded-xl shadow-lg border  border-zinc-700 backdrop-blur-sm">
                {/* Header */}
                <h2 className="text-xl sm:text-2xl font-bold text-center mb-4  text-zinc-200">
          ...
          {weeksError && <p className="text-xs text-red-500 mt-1">{weeksError}</p>}
                ...
                <div className="max-h-[110px] overflow-y-scroll scrollbar-hide flex flex-col justify-center items-center"> {/* Give minimum height to avoid layout shifts */}
                    {loading ? (
                        ...
                        <p className="text-center text-slate-500 dark:text-zinc-400 py-5 px-4">
                            No scores recorded for this week yet. Be the first!
                        </p>
                    ) : (
                        // Render the actual leaderboard list
                        <ul className="w-[94%] flex flex-col justify-center items-center space-y-3 animate-fade-in"> {/* Added subtle fade-in */}
                            {leaderboard.map((entry, index) => (
                                <li
                                    // Use a key that includes the week to ensure proper re-renders on week change
                                    key={entry.user_id + '-' + selectedWeekStart}
                                    className="flex items-center justify-between p-3 bg-slate-50 dark:bg-zinc-700/50 rounded-lg shadow-sm transition-all duration-150 hover:shadow-md hover:scale-[1.02] w-full"
                                >
                ...
                <Link href="/leaderboard" className="w-full flex flex-row sm:w-fit py-2.5 px-6 mt-3 mx-auto 
                    
                    dark:bg-zinc-800 dark:text-zinc-200 dark:hover:bg-zinc-200 dark:hover:text-zinc-800 
                    
                    bg-zinc-800 hover:bg-zinc-200 text-white hover:text-zinc-800
                                    
                                    font-medium rounded-lg shadow-md transition-all transform hover:scale-[1.03] focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 dark:focus:ring-offset-zinc-900 mb-4 text-center">
                    View Full Leaderboard <FiChevronRight className="inline ml-1" />
                </Link> {/* End Leaderboard Content Area */}
            </div>
        </div>
    );
}